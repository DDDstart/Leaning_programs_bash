...
# В Linux значения некоторых ключей стали чем-то вроде стандарта, которого полезно придерживаться. Вот список этих ключей:
...
# READ. Получение данных от пользователя
##
echo -n "Введите свое имя: " # ключ -n позволяет пользователю скрипта вводить данные там же, где расположено приглашение, а не на следующей строке
read name
echo "Привет, $name, добро пожаловать в мою программу"

# При вызове READ можно указывать и несколько переменных
##
read -p "Введите свое имя: " first last # ключ -p используется для отображения приглашения (prompt) перед вводом данных пользователем
echo "Вами введено: $first, $last..."

# Вызов READ без переменных
# Если не указывать переменную, то данные, введённые пользователем, будут помещены в специальную переменную среды REPLY
##
read -p "Введите свое имя: "
echo "Вами введено: $REPLY"

# Вызов READ с ожиданием по времени
##
if read -t 5 -p "Введите свое имя: " name # !!! Параметр -t должен указываться РАНЬШЕ параметра -p. Проверка по времени осушествляется в ЦИКЛЕ
then
   echo "Ваше имя: $name"
else
   echo "Время для ввода имени закончилось"
fi

# Ввод паролей
# Ключ -s команды READ предотвращает отображение на экране данных, вводимых с клавиатуры, делает цвет текста таким же, как цвет фона
##
read -s -p "Введите свой пароль: " pass
echo "Ваш пароль: $pass"

# Чтение данных из файла
## 
count=1
myfile=03_BASH_Ключи_командной_строки_file # Если имя файла сразу передать в CAT, то будет ошибка из=за русских букв
